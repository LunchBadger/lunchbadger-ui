...
const checkMovedConnection = info => (_, getState) => {
  ...
  if (isBackend && isPrivate) {
    const previousTargetConnections = storeUtils.filterConnections(state, {toId: storeUtils.formatId(newTargetId)});
    const previousSourceConnections = storeUtils.filterConnections(state, {fromId: storeUtils.formatId(newSourceId)});
